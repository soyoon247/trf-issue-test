module "cart_index_resources" {
  source      = "app.terraform.io/hh-devops/api-gateway-modules/aws"
  version     = "1.0.8"
  rest_api_id = var.args.rest_api_id
  parent_id   = var.parent_id
  path_part   = "{cart_index}"
  ...
  method_values = {
    ...
    PUT = {
      ...
      integration_request_parameters = {
        "integration.request.path.cart_index" = "method.request.path.cart_index"
        "integration.request.path.user_index" = "method.request.path.user_index"
      }
      integration_uri = "http://$${stageVariables.HWAHAE_COMMERCE_SERVER}/users/{user_index}/cart-items/{cart_index}"
      integration_response_map = {
        200 = {
          response_parameters = {
            "method.response.header.Access-Control-Allow-Origin" = var.args.all_origin
