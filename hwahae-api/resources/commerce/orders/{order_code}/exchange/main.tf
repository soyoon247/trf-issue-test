module "exchange_resources" {
  ...
  method_values = {
    ...
    POST = {
      ...
      integration_uri = "http://$${stageVariables.HWAHAE_COMMERCE_SERVER}/orders/{order_code}/exchange"
      integration_response_map = {
        200 = {
          response_parameters = {
            "method.response.header.Access-Control-Allow-Origin" = var.args.all_origin
...
module "calculate" {
  source    = "./calculate"
  parent_id = module.exchange_resources.resource_id
  args      = var.args
}
