module "rewards_resources" {
  ...
  method_values = {
    GET = {
      ...
      integration_uri = "http://$${stageVariables.HWAHAE_SERVER_API_ALB}/$${stageVariables.version}/weather-check/users/{user_id}/rewards"
    }
    OPTIONS = {
...
module "point_type_id" {
  source    = "./{point_type_id}"
  parent_id = module.rewards_resources.resource_id
  args      = var.args
}
